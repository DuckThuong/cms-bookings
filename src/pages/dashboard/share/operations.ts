...
export type TripStatusKey = 'scheduled' | 'boarding' | 'running' | 'completed' | 'delayed';
...
export const TRIP_STATUS_META: Record<TripStatusKey, StatusMeta> = {
  ...
  delayed: { label: 'Trễ chuyến', color: '#ef4444', bg: 'rgba(239,68,68,0.12)' },
};

export const ROUTE_STATUS_META: Record<RouteStatusKey, StatusMeta> = {
  active: { label: 'Khai thác ổn định', color: '#22c55e', bg: 'rgba(34,197,94,0.12)' },
  peak: { label: 'Nhu cầu cao', color: '#f97316', bg: 'rgba(249,115,22,0.12)' },
  reduced: { label: 'Giảm chuyến', color: '#f59e0b', bg: 'rgba(245,158,11,0.12)' },
...
export const VEHICLE_STATUS_META: Record<VehicleStatusKey, StatusMeta> = {
  ...
  in_service: { label: 'Đang khai thác', color: '#3b82f6', bg: 'rgba(59,130,246,0.12)' },
  maintenance: { label: 'Bảo dưỡng', color: '#ef4444', bg: 'rgba(239,68,68,0.12)' },
  idle: { label: 'Chờ phân công', color: '#f59e0b', bg: 'rgba(245,158,11,0.12)' },
};
...
export const getTripSummary = (data: TripRecord[]): SummaryItem[] => [
  ...
  { key: 'running', label: 'Đang chạy', color: '#22c55e', value: data.filter((item) => item.status === 'running').length },
  { key: 'boarding', label: 'Đang đón khách', color: '#f59e0b', value: data.filter((item) => item.status === 'boarding').length },
  { key: 'load', label: 'Lấp đầy TB', color: '#f97316', value: `${Math.round(data.reduce((sum, item) => sum + item.occupancyRate, 0) / data.length)}%` },
];
...
export const getRouteSummary = (data: RouteRecord[]): SummaryItem[] => [
  ...
  { key: 'tripsPerDay', label: 'Chuyến/ngày', color: '#22c55e', value: data.reduce((sum, item) => sum + item.tripsPerDay, 0) },
  { key: 'occupancy', label: 'Lấp đầy TB', color: '#eab308', value: `${Math.round(data.reduce((sum, item) => sum + item.averageOccupancy, 0) / data.length)}%` },
];
