...
const CompanyRegistrationsPage = () => {
  ...
  const updateStatusMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setRejectReason("");
      setDrawerOpen(false);
      void queryClient.invalidateQueries({ queryKey: ["company-registrations"] });
    },
    onError: () => {
  ...
  const dataWithKeys = useMemo(
    () => registrations.map((item) => ({ ...item, key: `registration-${item.id}` })),
    [registrations],
  );
  ...
  const handleApprove = (record: CompanyRegistrationRecord) => {
    Modal.confirm({
      ...
      cancelText: "Hủy",
      onOk: () => {
        updateStatusMutation.mutate({ id: record.id, payload: { status: RegistrationStatus.APPROVED } });
      },
    });
  };
  ...
  const handleReject = () => {
    ...
    updateStatusMutation.mutate({ id: selectedRecord.id, payload: { status: RegistrationStatus.REJECTED, rejectionReason: rejectReason } });
  };

  const columns: ColumnsType<CompanyRegistrationRecord> = [
    { title: "Mã", dataIndex: "id", key: "id", width: 80, render: (value: number) => (
      <span style={{ fontFamily: "monospace", fontWeight: 600 }}>{value}</span>
    )},
    { title: "Tên nhà xe", dataIndex: "companyName", key: "companyName", render: (value: string) => <strong>{value}</strong> },
    { title: "Người đăng ký", key: "user", render: (_, record) => (
      <div>
    ...
    { title: "", key: "actions", width: 180, render: (_, record) => (
      <div className="row-actions">
        <Button type="primary" icon={<EyeOutlined />} onClick={(e) => { e.stopPropagation(); handleViewDetail(record); }} />
        {record.status === RegistrationStatus.PENDING && (
          <>
            <Button type="primary" icon={<EditOutlined />} style={{ background: "#22c55e", borderColor: "#22c55e" }} onClick={(e) => { e.stopPropagation(); handleApprove(record); }} />
            <Button type="primary" danger icon={<DeleteOutlined />} onClick={(e) => { e.stopPropagation(); setSelectedRecord(record); setRejectReason(""); setRejectModalOpen(true); }} />
          </>
        )}
      </div>
    )},
  ...
  return (
    <div className="mgmt-page">
      <div className="mgmt-hero">
        ...
        <div className="mgmt-hero__subtitle">Xem xét và phê duyệt yêu cầu đăng ký trở thành nhà xe của người dùng.</div>
      </div>
      ...
      <div className="bm-content">
        <div className="bm-table-wrap bm-table">
          <Table rowKey="key" columns={columns} dataSource={filtered} pagination={{ pageSize: 8, showSizeChanger: false }} loading={isLoading} onRow={(record) => ({ onClick: () => handleViewDetail(record) })} />
        </div>
      </div>

      <Drawer className="booking-drawer" open={Boolean(selectedRecord && drawerOpen)} onClose={() => setDrawerOpen(false)} width={520} title={selectedRecord ? `Yêu cầu #${selectedRecord.id}` : ""}>
        {selectedRecord && (
          <div className="drawer-body">
            <div className="drawer-body__section">
              ...
              <div className="mgmt-detail-list">
                <div className="mgmt-detail-list__item"><span className="mgmt-detail-list__label">Tên nhà xe</span><span className="mgmt-detail-list__value">{selectedRecord.companyName}</span></div>
                <div className="mgmt-detail-list__item"><span className="mgmt-detail-list__label">Người đăng ký</span><span className="mgmt-detail-list__value">{selectedRecord.userName}</span></div>
                <div className="mgmt-detail-list__item"><span className="mgmt-detail-list__label">Số điện thoại</span><span className="mgmt-detail-list__value">{selectedRecord.userPhone}</span></div>
                <div className="mgmt-detail-list__item"><span className="mgmt-detail-list__label">Email</span><span className="mgmt-detail-list__value">{selectedRecord.userEmail}</span></div>
                <div className="mgmt-detail-list__item"><span className="mgmt-detail-list__label">Địa chỉ</span><span className="mgmt-detail-list__value">{selectedRecord.address || "Chưa cập nhật"}</span></div>
                <div className="mgmt-detail-list__item"><span className="mgmt-detail-list__label">Người đại diện</span><span className="mgmt-detail-list__value">{selectedRecord.representativeName || "Chưa cập nhật"}</span></div>
                <div className="mgmt-detail-list__item"><span className="mgmt-detail-list__label">Chức vụ</span><span className="mgmt-detail-list__value">{selectedRecord.representativePosition || "Chưa cập nhật"}</span></div>
                <div className="mgmt-detail-list__item"><span className="mgmt-detail-list__label">Số điện thoại ĐD</span><span className="mgmt-detail-list__value">{selectedRecord.representativePhone || "Chưa cập nhật"}</span></div>
                <div className="mgmt-detail-list__item"><span className="mgmt-detail-list__label">Mã số thuế</span><span className="mgmt-detail-list__value">{selectedRecord.taxCode || "Chưa cập nhật"}</span></div>
                <div className="mgmt-detail-list__item"><span className="mgmt-detail-list__label">Địa chỉ GPKD</span><span className="mgmt-detail-list__value">{selectedRecord.businessAddress || "Chưa cập nhật"}</span></div>
            ...
            {selectedRecord.status === RegistrationStatus.PENDING && (
              <div style={{ justifySelf: "center", marginTop: 24 }}>
                <div style={{ display: "flex", gap: 8 }}>
                  <Button className="btn-primary" icon={<EditOutlined />} style={{ background: "#22c55e", borderColor: "#22c55e" }} onClick={() => handleApprove(selectedRecord)}>Phê duyệt</Button>
                  <Button danger icon={<DeleteOutlined />} onClick={() => { setRejectReason(""); setRejectModalOpen(true); }}>Từ chối</Button>
                </div>
              </div>
            )}
          </div>
      ...
      <Modal title="Từ chối đăng ký" open={rejectModalOpen} onCancel={() => setRejectModalOpen(false)} onOk={handleReject} okText="Từ chối" cancelText="Hủy" okButtonProps={{ danger: true }}>
        <div style={{ marginTop: 16 }}>
          <p>Vui lòng nhập lý do từ chối để thông báo cho người dùng:</p>
          <Input.TextArea rows={4} value={rejectReason} onChange={(e) => setRejectReason(e.target.value)} placeholder="Nhập lý do từ chối..." />
        </div>
      </Modal>
    </div>
  );
